...
export class UserService {
  ...
  async upsert(data: UpsertUserInput): Promise<UserId> {
    ...
    try {
      ...
      this.errorManagerService.logErrorAndThrow(error, UserService.name, MessageAction.UPSERT, MessageEntity.USER, {
  ...
  async delete(id: string): Promise<UserId> {
    try {
      ...
      this.errorManagerService.logErrorAndThrow(error, UserService.name, MessageAction.DELETE, MessageEntity.USER, {
        clerkUserId: id,
      });
    }
  }
